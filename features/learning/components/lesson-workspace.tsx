...
export function LessonWorkspace({
  ...
  const trackLabel =
    currentTrack === 'umum' ? course.level : (activeTrack?.label ?? lesson.title);
                ...
                {syllabus.map((item, index) => {
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        'flex items-start gap-3 border-b border-border/60 px-4 py-3 last:border-none transition-colors hover:bg-muted/30',
            ...
            <div className="space-y-4">
              <div className="relative overflow-hidden rounded-2xl border border-border bg-black shadow-md">
                {lesson.videoUrl ? (
                  ...
                  <div className="flex aspect-video flex-col items-center justify-center gap-3 bg-linear-to-br from-secondary to-background px-6 text-center">
                    <Play className="size-12 text-primary" />
                    <p className="text-sm text-muted-foreground">
                      Video belum tersedia — baca ringkasan materi di bawah.
                    </p>
                  </div>
                )}
                <div className="absolute top-4 left-4">
                  <Badge className={activeTrackStyle}>{course.level}</Badge>
                </div>
              </div>
