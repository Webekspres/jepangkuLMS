...
import { ArrowRight, ClipboardCheck, Compass, PlayCircle, RotateCcw } from 'lucide-react';
...
export function PlacementHubPage({ latestAttempt, inProgress }: PlacementHubPageProps) {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 pb-8 sm:space-y-5">
      ...
      {inProgress ? (
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-4 shadow-sm sm:px-5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-4">
            <div className="flex items-center gap-3">
              <div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-amber-500/15 text-amber-700">
                <PlayCircle className="size-4" />
              </div>
              <div>
                <p className="text-sm font-bold text-foreground">Tes belum selesai</p>
                <p className="text-xs text-muted-foreground">
                  {inProgress.answeredCount}/{totalQuestions} soal terjawab
                  {' · '}
                  terakhir{' '}
                  {inProgress.updatedAt.toLocaleString('id-ID', {
                    day: 'numeric',
                    month: 'short',
                    hour: '2-digit',
                    minute: '2-digit',
